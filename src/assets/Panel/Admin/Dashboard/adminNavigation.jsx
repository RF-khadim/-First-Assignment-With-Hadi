...
const AdminNavigation = () => {
    ...
    return (
        <div
            className={` bg-slate-600 h-screen pl-3 relative ${open ? "w-64" : "w-20"}`}>
            <div className='flex flex-col items-center'>
                ...
                <div onClick={() => setOpen(!open)} className='w-7 h-7 bg-slate-950 rounded-full top-28 left-[242px] absolute'>
                ...
                <div className='text-white mt-20 font-semibold '>
                    {AdminNavLinks.map((item, index) => (
                        <div className={"my-8 flex py-[7px] pl-2 " + (activeLink === index ? "bg-white text-slate-950 rounded" : "")}
                            onClick={() => { setActiveLink(index) }}
                            key={index}>
                            <span className={`pt-1 text-2xl   ${open ? "pr-10" : " pl-2 pr-3"}  `}> {item.icon}</span>
                            <span className={`pr-12 mt-1 ${open ? "block" : "hidden"}`}>{item.name}</span></div>
                    ))}
